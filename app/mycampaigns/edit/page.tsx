...
export default function Edit() {
  ...
  const [description, setDescription] = useState(
    myCampaigns[index]?.description
  );
  const [missionType, setMissionType] = useState(
    myCampaigns[index]?.mission_type
  );
  ...
  const [recruitStartDate, setRecruitStartDate] = useState<any>(
    new Date(myCampaigns[index]?.recruit_start_date)
  );
  const [recruitEndDate, setRecruitEndDate] = useState<any>(
    new Date(myCampaigns[index]?.recruit_end_date)
  );

  const [startDate, setStartDate] = useState<any>(
    new Date(myCampaigns[index]?.start_date)
  );
  const [endDate, setEndDate] = useState<any>(
    new Date(myCampaigns[index]?.end_date)
  );
  const [shownStartDate, setShownStartDate] = useState<string>(
    myCampaigns[index]?.start_date
  );
  const [shownEndDate, setShownEndDate] = useState<string>(
    myCampaigns[index]?.end_date
  );
    ...
    if (startDateArr?.[2] !== endDateArr?.[2]) {
      ...
      if (endDateArr?.[0] > startDateArr?.[0]) {
        if (Number(endDateArr?.[1]) + 12 - Number(startDateArr?.[1]) === 3) {
          ...
          Number(endDateArr?.[1]) + 12 - Number(startDateArr?.[1]) ===
          6
        ) {
          setPeriod("6 months");
        } else {
